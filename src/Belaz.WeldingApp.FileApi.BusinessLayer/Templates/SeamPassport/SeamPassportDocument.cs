...
public class SeamPassportDocument : IDocument
{
    ...
    public void Compose(IDocumentContainer container)
    {
        ...
        var timeNewRomanItalicBoldFontPath = Path.Combine(_fontsPath, "times-new-roman-italic-bold.ttf");
    ...
    private void ComposeProductionTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Cell().ColumnSpan(2).Element(BlockCenter).Text("Паспорт сварного шва").Style(Typography.Bold);

            table.Cell().Row(2).Column(1).Element(BlockLeft).Text("Наименование предприятия / организации")
                .Style(Typography.Normal);
            table.Cell().Row(2).Column(2).Element(BlockLeft).Text("-").Style(Typography.Italic);

            table.Cell().Row(3).Column(1).Element(BlockLeft).Text("Наименование цеха").Style(Typography.Normal);
            table.Cell().Row(3).Column(2).Element(BlockLeft)
                .Text($"{Task.Seam.Workshop.Name} №{Task.Seam.Workshop.Number}").Style(Typography.Italic);

            table.Cell().Row(4).Column(1).Element(BlockLeft).Text("Наименование участка").Style(Typography.Normal);
            table.Cell().Row(4).Column(2).Element(BlockLeft)
                .Text($"{Task.Seam.ProductionArea.Name} №{Task.Seam.ProductionArea.Number}").Style(Typography.Italic);
    ...
    private void ComposeDataWeldingTaskTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Cell().Column(1).Element(BlockLeft).Text("Дата выполнения сварки")
                .Style(Typography.Normal);
            table.Cell().Column(2).Element(BlockLeft).Text(Task.WeldingDate).Style(Typography.Italic);
    ...
    private void ComposeProductsInfoTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Cell().Column(1).Element(BlockLeft).Text("Наименование изделия")
                .Style(Typography.Normal);
            ...
            table.Cell().Row(2).Column(1).Element(BlockLeft).Text("Наименование узла")
                .Style(Typography.Normal);
            var knotInfo = GetProductInfo(Task.Seam.Knot);
            table.Cell().Row(2).Column(2).Element(BlockLeft).Text(knotInfo).Style(Typography.Italic);

            table.Cell().Row(3).Column(1).Element(BlockLeft).Text("Наименование детали")
                .Style(Typography.Normal);
            var detailInfo = GetProductInfo(Task.Seam.Detail);
            table.Cell().Row(3).Column(2).Element(BlockLeft).Text(detailInfo).Style(Typography.Italic);

            table.Cell().Row(4).Column(1).Element(BlockLeft).Text("Номер сварного шва")
                .Style(Typography.Normal);
            table.Cell().Row(4).Column(2).Element(BlockLeft).Text($"№{Task.Seam.Number}").Style(Typography.Italic);
    ...
    private void ComposeMasterAndWelderTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Cell().Column(1).Element(BlockLeft).Text("Руководитель сварочных работ (мастер)")
                .Style(Typography.Normal);
            table.Cell().Column(2)
                .Element(BlockLeft)
            ...
            table.Cell().Row(2).Column(1).Element(BlockLeft)
            ...
            table.Cell().Row(3).Column(1).Element(BlockLeft).Text("Сварщик")
                .Style(Typography.Normal);
            table.Cell().Row(3).Column(2)
                .Element(BlockLeft)
                .Text($"{Task.Welder.MiddleName} {Task.Welder.FirstName} {Task.Welder.LastName}")
                .Style(Typography.Italic);

            table.Cell().Row(4).Column(1).Element(BlockLeft).Text("Срок действия удостоверения")
                .Style(Typography.Normal);
    ...
    private void ComposeWeldingEquipmentTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Cell().Column(1).Element(BlockLeft).Text("Наименование оборудования")
                .Style(Typography.Normal);
            table.Cell().Column(2)
                .Element(BlockLeft)
                .Text($"{Task.WeldingEquipment.Name}")
                .Style(Typography.Italic);

            table.Cell().Row(2).Column(1).Element(BlockLeft).Text("Инвентарный номер оборудования")
                .Style(Typography.Normal);
            table.Cell().Row(2).Column(2)
                .Element(BlockLeft)
                .Text($"№{Task.WeldingEquipment.FactoryNumber}")
                .Style(Typography.Italic);

            table.Cell().Row(3).Column(1).Element(BlockLeft).Text("Дата очередной аттестации (ППР)")
                .Style(Typography.Normal);
            table.Cell().Row(3).Column(2)
                .Element(BlockLeft)
                .Text($"{Task.WeldingEquipment.NextAttestationDate}")
                .Style(Typography.Italic);
    ...
    private void ComposeTechnologicalInstructionTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Cell().Column(1).Element(BlockLeft).Text("Инструкция на технологический процесс сварки")
                .Style(Typography.Normal);
            table.Cell().Column(2)
                .Element(BlockLeft)
                .Text($"{Task.Seam.TechnologicalInstruction.Name} №{Task.Seam.TechnologicalInstruction.Number}")
                .Style(Typography.Italic);
    ...
    private void ComposeWeldPassageInstructionsTable(IContainer container)
    {
        container.Column(column =>
        {
            ...
            column.Item().Table(table =>
            {
                ...
                table.Header(header =>
                {
                    table.Cell().RowSpan(2).Element(BlockLeft).Text("№ п/п")
                        .Style(Typography.Normal);
                ...
                foreach (var weldPassageInstructions in Task.Seam.TechnologicalInstruction.WeldPassageInstructions
                             .OrderBy(_ => _.Number))
                {
                    table.Cell()
                        .Element(BlockLeft)
                    ...
                    table.Cell()
                        .Element(BlockLeft)
                        .Text(weldPassageInstructions.PreheatingTemperatureMin.ToString(CultureInfo.InvariantCulture))
                        .Style(Typography.Normal);

                    table.Cell()
                        .Element(BlockLeft)
                        .Text(weldPassageInstructions.PreheatingTemperatureMax.ToString(CultureInfo.InvariantCulture))
                        .Style(Typography.Normal);
    ...
    private void ComposeAdditionalInfoTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Cell().Element(BlockLeft).Text("Основной материал")
                .Style(Typography.Normal);
            table.Cell()
                .Element(BlockLeft)
            ...
            table.Cell().Element(BlockLeft).Text("№ сертификата (партии) основного материала")
                .Style(Typography.Normal);
            table.Cell()
                .Element(BlockLeft)
                .Text($"№ {Task.MainMaterialBatchNumber}")
                .Style(Typography.Italic);

            table.Cell().Element(BlockLeft).Text("Сварочные материалы")
                .Style(Typography.Normal);
            table.Cell()
                .Element(BlockLeft)
            ...
            table.Cell().Element(BlockLeft).Text("№ сертификата (партии) св. материала")
                .Style(Typography.Normal);
            table.Cell()
                .Element(BlockLeft)
            ...
            table.Cell().Element(BlockLeft).Text("Защитный газ")
                .Style(Typography.Normal);
            table.Cell()
                .Element(BlockLeft)
            ...
            table.Cell().Element(BlockLeft).Text("№ сертификата (партии) на защитный газ")
                .Style(Typography.Normal);
            table.Cell()
                .Element(BlockLeft)
                .Text(Task.ProtectiveGasBatchNumber is not null ? $"№ {Task.ProtectiveGasBatchNumber}" : "-")
                .Style(Typography.Italic);
    ...
    private void ComposeInspectorTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            table.Cell().Element(BlockLeft).Text("Контролёр")
                .Style(Typography.Normal);
            table.Cell()
                .Element(BlockLeft)
