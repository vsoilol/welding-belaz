...
namespace Belaz.WeldingApp.FileApi.BusinessLayer.Helpers.Implementations;
...
public class MarkEstimateService : IMarkEstimateService
{
    ...
    public double? GetAverageEstimation(double[] weldingCurrents, double[] voltages, double? minAllowanceWeldingCurrent,
        double? maxAllowanceWeldingCurrent, double? minAllowanceVoltage, double? maxAllowanceVoltage)
    {
        if (IsAllowancesNullOrEqualZero(minAllowanceWeldingCurrent, maxAllowanceWeldingCurrent, minAllowanceVoltage,
                maxAllowanceVoltage))
        {
            return null;
        }
